...
const GherkinVisualizer: React.FC<{ rawText: string }> = ({ rawText }) => {
  ...
  return (
    <div ref={containerRef} className="w-full h-full min-h-[500px] overflow-auto bg-slate-950/20 rounded-3xl border border-cyan-500/10 p-8 relative">
        ...
        <div className="relative flex justify-center gap-12 w-full pt-10">
          ...
          {feature.scenarios.map((scenario, sIdx) => (
            <div key={sIdx} className="relative flex flex-col items-center w-full max-w-[320px]">
              <div className="bg-slate-900/80 backdrop-blur-md border border-slate-700/50 p-6 rounded-3xl w-full hover:border-cyan-500/50 transition-all duration-500 shadow-xl group">
                ...
                <h5 className="text-white font-bold text-sm mb-6 leading-snug group-hover:text-cyan-400 transition-colors">{scenario.name}</h5>
...
export const AITools: React.FC = () => {
  ...
  return (
    <section id="ai-tools" className="py-32 px-4 bg-slate-950 border-y border-cyan-500/10">
      <div className="max-w-7xl mx-auto">
        <div className="mb-16 flex flex-col md:flex-row justify-between items-end gap-8">
          <div className="max-w-xl">
            <h2 className="text-4xl font-black mb-4 text-white tracking-tighter uppercase">Security Intelligence Engine</h2>
            <p className="text-slate-400 text-lg">AI-powered auditor for automated vulnerability research and architectural modeling.</p>
          </div>
          <div className="px-4 py-2 bg-cyan-500/10 border border-cyan-500/30 rounded text-cyan-500 font-mono text-[10px] uppercase tracking-widest">
            MODEL: GEMINI_3_FLASH // STATUS: READY
          </div>
        </div>

        <div className="grid lg:grid-cols-12 gap-10 items-stretch">
          <div className="lg:col-span-5 bg-slate-900 border border-slate-800 rounded-3xl p-8 flex flex-col">
            <div className="flex p-1 bg-slate-950 rounded-xl mb-8 flex-wrap gap-1 border border-slate-800">
              {[
                ...
                <button 
                  ...
                  className={`px-4 py-2 rounded-lg text-[9px] font-black tracking-widest uppercase transition-all ${activeTab === tab.id ? 'bg-cyan-500 text-slate-950 shadow-[0_0_20px_rgba(6,182,212,0.3)]' : 'text-slate-500 hover:text-slate-300'}`}
                >
                  {tab.label}
                </button>
              ))}
            ...
            <div className="flex-grow flex flex-col gap-6">
              <div className="flex justify-between items-center">
                <label className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Target Payload</label>
                <button onClick={() => setInput(SAMPLES[activeTab])} className="text-[10px] font-black text-cyan-500 hover:text-cyan-400">LOAD_SAMPLE.dat</button>
              </div>
              <textarea
                value={input}
                onChange={(e) => setInput(e.target.value)}
                className="w-full flex-grow bg-slate-950 border border-slate-800 rounded-2xl p-6 text-sm mono text-cyan-500/90 focus:border-cyan-500 focus:ring-1 focus:ring-cyan-500 outline-none placeholder-slate-800 resize-none min-h-[300px]"
                placeholder="Enter protocol requirements or code snippets..."
              />
              <button
                onClick={handleProcess}
                disabled={loading || !input.trim()}
                className="w-full py-5 bg-cyan-500 hover:bg-cyan-400 disabled:bg-slate-800 text-slate-950 font-black uppercase tracking-widest transition-all rounded-2xl relative overflow-hidden"
              >
                {loading && (
                ...
                <span className="relative z-10">{loading ? LOADING_MESSAGES[activeTab][loadingStage] : "Execute Audit"}</span>
              </button>
            </div>
          </div>

          <div className="lg:col-span-7 bg-slate-950 border border-slate-800 rounded-3xl flex flex-col overflow-hidden relative shadow-2xl">
            <div className="p-5 border-b border-slate-800 flex justify-between items-center bg-slate-900/50">
              <div className="flex gap-4 items-center">
                <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Output_Buffer</span>
                {activeTab === AIToolType.GHERKIN_GEN && output && (
                  <div className="flex bg-slate-950 rounded-lg p-0.5 border border-slate-800">
                    <button onClick={() => setViewMode('CODE')} className={`px-3 py-1 rounded text-[9px] font-black transition-all ${viewMode === 'CODE' ? 'bg-cyan-500 text-slate-950' : 'text-slate-500'}`}>CODE</button>
                    <button onClick={() => setViewMode('VISUAL')} className={`px-3 py-1 rounded text-[9px] font-black transition-all ${viewMode === 'VISUAL' ? 'bg-cyan-500 text-slate-950' : 'text-slate-500'}`}>MAP</button>
                  </div>
                )}
              </div>
              {output && (
                <button onClick={() => { navigator.clipboard.writeText(output); setCopied(true); setTimeout(()=>setCopied(false),2000); }} className="text-[10px] font-black text-cyan-500 hover:text-cyan-400">
                  {copied ? 'BUFFER_COPIED' : 'DOWNLOAD_LOGS'}
                </button>
              )}
            </div>
            ...
            <div className="flex-grow flex flex-col overflow-hidden">
              {activeTab === AIToolType.ARCH_COPILOT && output && (
                 <div ref={mermaidRef} className="p-8 bg-white/5 border-b border-slate-800 min-h-[300px] flex items-center justify-center overflow-x-auto relative">
                    <div className="absolute top-4 left-4 text-[8px] font-black text-cyan-500/40 uppercase tracking-widest">Topology Visualization</div>
                 </div>
              )}
              <div className="flex-grow overflow-y-auto max-h-[600px] relative">
                {output ? (
                  viewMode === 'VISUAL' && activeTab === AIToolType.GHERKIN_GEN ? (
                    ...
                    <div className="p-8 text-sm mono leading-relaxed text-slate-300 whitespace-pre-wrap selection:bg-cyan-500/20">{output.replace(/```mermaid[\s\S]*?```/g, '')}</div>
                  )
                ) : (
                  <div className="h-full flex flex-col items-center justify-center text-slate-800 space-y-4 py-24">
                    {loading ? (
                      <div className="flex flex-col items-center gap-6">
                        <div className="w-12 h-12 border-2 border-cyan-500/20 border-t-cyan-500 rounded-full animate-spin"></div>
                        <span className="text-[10px] font-black text-cyan-500 uppercase tracking-widest animate-pulse">{LOADING_MESSAGES[activeTab][loadingStage]}</span>
                      </div>
                    ) : (
                      <>
                        <div className="w-20 h-20 border border-slate-900 rounded-full flex items-center justify-center opacity-20">
                           <svg className="w-10 h-10" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 15l-3-3m0 0l3-3m-3 3h8M5 12h1m14 0h1" strokeWidth="1" strokeLinecap="round" strokeLinejoin="round"/></svg>
                        </div>
                        <p className="text-[10px] font-black uppercase tracking-widest">System standby. Awaiting payload.</p>
                      </>
                    )}
                  </div>
                )}
